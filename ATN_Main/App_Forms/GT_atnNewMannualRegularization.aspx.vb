Partial Class GT_NewMannualRegularization
  ...
  Protected Sub LC_CardNo1_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles LC_CardNo1.TextChanged
    ...
    If Convert.ToInt32(oEmp.C_OfficeID) <> hrmOffices.Site And Convert.ToInt32(Session("FinYear")) >= 2021 Then
      newRule2021 = True
    End If
    'If Convert.ToBoolean(ConfigurationManager.AppSettings("Demo")) Then
    '  If Convert.ToInt32(oEmp.C_OfficeID) <> enumOffices.Site Then
    '    newRule2021 = True
    '  End If
    'End If
  ...
  Private Function LoadIncompleteAttendance(oEmp As SIS.ATN.atnEmployees, newRule2021 As Boolean) As String
    ...
    Dim oInAtns As List(Of SIS.ATN.atnProcessedPunch) = SIS.ATN.atnProcessedPunch.NewGetAllIncompleteAttendanceWithoutFilter(CardNo)
    ...
    For Each oAt As SIS.ATN.atnProcessedPunch In oInAtns
      ...
      If Convert.ToDateTime(oAt.AttenDate) >= Convert.ToDateTime("01/01/2021") Then
        Select Case Convert.ToInt32(OfficeID)
          Case hrmOffices.Chennai, hrmOffices.Site, hrmOffices.Kolkata, hrmOffices.Mumbai
            If Convert.ToDecimal(oAt.Punch1Time > 0 And oAt.Punch2Time > 0) Then
              If oAt.PunchStatusID = "AS" Or oAt.PunchStatusID = "AF" Or oAt.PunchStatusID = "TS" Then
                ...
                If Convert.ToDecimal(oAt.Punch2Time) >= 18.15 Then
                  Dim hrs As Decimal = SIS.SYS.Utilities.NewAttendanceRules.DiffTime(oAt.Punch1Time, 18.15)
                  If hrs >= 7.15 Then
                    SHEnabled = True
                  End If
                Else
          ...
          Case Else
            If Convert.ToDecimal(oAt.Punch1Time > 0 And oAt.Punch2Time > 0) Then
              If oAt.PunchStatusID = "AS" Or oAt.PunchStatusID = "AF" Or oAt.PunchStatusID = "TS" Then
                'For Morning Short Leave
                If Convert.ToDecimal(oAt.Punch2Time) >= 18.0 Then
                  Dim hrs As Decimal = SIS.SYS.Utilities.NewAttendanceRules.DiffTime(oAt.Punch1Time, 18.0)
                  If hrs >= 7.15 Then
                    SHEnabled = True
                  End If
                Else
  ...
  Private Function DrawTblDates(ByVal oLTs As List(Of SIS.ATN.atnLeaveTypes), newRule2021 As Boolean) As Integer
    ...
    ScriptManager.RegisterClientScriptBlock(Page, GetType(System.String), "AdvanceLeaveTypeChanged", mStr, False)
    Return tblWidth
  End Function
